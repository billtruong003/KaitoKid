    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 ApplyInteraction(float3 worldPos, float3 instCenter, float3 localPos, float meshH, float springVal)
            {
                ...
                if (_PlayerPartCount >= 1.0) CalcPointPush(instCenter, _PlayerPos0.xyz, hRatio, pushXZ);
                if (_PlayerPartCount >= 2.0) CalcPointPush(instCenter, _PlayerPos1.xyz, hRatio, pushXZ);
                if (_PlayerPartCount >= 3.0) CalcSegmentPush(instCenter, _PlayerPos0.xyz, _PlayerPos1.xyz, hRatio, pushXZ);
                worldPos.xz += pushXZ;
        ...
        Pass
        {
            ...
            half4 ShadowFrag(Varyings input) : SV_Target { return half4(0, 0, 0, 0); }
            ENDHLSL
        }
        ...
        Pass
        {
            ...
            half4 DepthFrag(Varyings input) : SV_Target { return half4(0, 0, 0, 0); }
            ENDHLSL
        }
    }
}
