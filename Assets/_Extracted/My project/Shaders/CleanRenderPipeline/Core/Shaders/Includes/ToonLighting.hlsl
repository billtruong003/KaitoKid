// ============================================================================
// ToonLighting.hlsl — Clean Render Pipeline v2.0 (WebGL-safe)
//
...
{
    half rim = (half)1.0 - saturate(NdotV);
    return smoothstep((half)1.0 - rcp(max(rimPower, (half)0.01)), (half)1.0, rim * lightIntensity);
}
    ...
    half3 rimColor, half rimPower, half rimIntensity, half ambientScale)
{
    ...
    result.rim = half3(0, 0, 0);
    result.globalIllumination = half3(0, 0, 0);
...
{
    half3 addColor = half3(0.0, 0.0, 0.0);
...
ToonLightingOutput ComputeToonLighting(ToonLightingInput input)
{
    ...
    half3 toonTint = lerp(input.shadowColor, half3(1.0, 1.0, 1.0), combinedLight);
    half3 directDiffuse = toonTint * (half3)mainLight.color * input.albedo;

    half3 additionalDiffuse = half3(0.0, 0.0, 0.0);
    #if defined(_ADDITIONAL_LIGHTS)
    {
        uint lightCount = GetAdditionalLightsCount();
        for (uint i = 0u; i < lightCount; i++)
    ...
    half3 shadowColor, half threshold, half smoothness)
{
    ...
    half3 tint = lerp(shadowColor, half3(1.0, 1.0, 1.0), shadow);
