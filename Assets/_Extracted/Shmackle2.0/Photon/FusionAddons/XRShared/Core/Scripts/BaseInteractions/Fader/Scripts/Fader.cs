...
namespace Fusion.XR.Shared.Locomotion
{
    ...
    public class Fader : MonoBehaviour, ICameraFader
    {
        ...
        void CreateFader()
        {
            ...
            target.material = new Material(Shader.Find("Unlit/Fader"));
        ...
        private void OnValidate()
        {
            ...
            ValidationUtils.SceneEditionValidate(gameObject, () => {
                ...
                bool found = false;
                const string shaderCollectionName = "FaderShaderCollection";
                for (int i = 0; i < preloadedShaderCollections.arraySize; i++)
                {
                    var c = preloadedShaderCollections.GetArrayElementAtIndex(i);
                    if (c?.objectReferenceValue is ShaderVariantCollection collection && collection.name == shaderCollectionName)
                    {
                        found = true;
                        break;
                    }
                ...
                if (found == false)
                {
                    Debug.LogError($"[Fader] To be sure that the fader shader is included in builds, add {shaderCollectionName} to the preloaded shaders in graphics settings");
                }
            });
#endif
        }
