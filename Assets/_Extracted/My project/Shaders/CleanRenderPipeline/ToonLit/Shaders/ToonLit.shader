Shader "CleanRender/ToonLit"
{
    ...
    SubShader
    {
        ...
        // ── Platform guard: procedural instancing needs StructuredBuffer (SSBO) ──
        // WebGL 2.0 / GLES3 does NOT support SSBO, so we completely skip
        // InstancingCore, StructuredBuffer declarations, and procedural:Setup.
        #if !defined(SHADER_API_GLES3) && !defined(SHADER_API_GLES)
            #define CLEAN_HAS_SSBO 1
        ...
        #if defined(UNITY_PROCEDURAL_INSTANCING_ENABLED) && defined(CLEAN_HAS_SSBO)
        ...
        Pass
        {
            ...
            // Procedural instancing only on platforms with SSBO support
            ...
            Varyings ToonLitVert(Attributes input)
            {
                ...
                o.normalWS = half3(0, 0, 0);
                o.uv = float4(0, 0, 0, 0);
                o.fogFactor = (half)0.0;
            ...
            half4 ToonLitFrag(Varyings input) : SV_Target
            {
                ...
                float3 viewDirWS = normalize(GetCameraPositionWS() - input.positionWS);
                    ...
                    ToonLightResult lit = ComputeToonMainLight(
                        ...
                        _RimColor.rgb, _RimPower, _RimColor.a, (half)1.0);
                #else
        ...
        Pass
        {
            ...
            half4 DNFrag(Varyings input) : SV_Target
            {
                ...
                return half4(n * (half)0.5 + (half)0.5, 0);
            }
            ENDHLSL
        }

        // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        // PASS 4: Meta (lightmap baking — editor only, no instancing)
        // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        Pass
        {
            Name "Meta"
