...
public class TerrainHoleCutter : EditorWindow
{
    ...
    int maxSubdivLevel = 5; // cap subdivision
    ...
    // ════════════════════════════════════════════════════════════════
    // GPU Preview (unchanged — HoleCutPreview.shader)
    // ════════════════════════════════════════════════════════════════
    ...
    //   - Fully solid → keep as-is
    //   - Fully hole → remove
    //   - Boundary → subdivide into NxN barycentric grid,
    //     classify each sub-tri, keep solid ones.
    //     Adjacent up+down pairs = quads.
    // ════════════════════════════════════════════════════════════════
    ...
    string CutSingle(GameObject go, MeshFilter mf, Texture2D tex, Vector4 st)
    {
        ...
        for (int t = 0; t < sTri.Length / 3; t++)
        {
            ...
            // Quick classify: sample 3 verts + centroid
            ...
            if (solidCount == 4)
            {
                // Fully solid → keep original triangle
                mb.AddTri(
            ...
            // Compute subdivision level from texel coverage
            ...
            // Build barycentric grid vertices
            // Index: gridVert[i][j] where i+j <= N, k = N-i-j
            int gridW = N + 1;
            int[,] gridIdx = new int[gridW, gridW]; // stores mb vertex index
            ...
            // Emit sub-triangles, classify each
            for (int gi = 0; gi < N; gi++)
            {
                for (int gj = 0; gj < N - gi; gj++)
                {
                    // "Up" triangle: (gi,gj) → (gi+1,gj) → (gi,gj+1)
                    ...
                    // "Down" triangle (if exists): (gi+1,gj) → (gi+1,gj+1) → (gi,gj+1)
                    ...
                    // Quad merge: if both up + down are solid → emit as quad (2 tris)
    ...
    // ════════════════════════════════════════════════════════════════
    // MeshBuilder — accumulates vertices + triangles
    // ════════════════════════════════════════════════════════════════
    ...
    class MeshBuilder
    {
        ...
        // Reuse cache for original vertex indices
