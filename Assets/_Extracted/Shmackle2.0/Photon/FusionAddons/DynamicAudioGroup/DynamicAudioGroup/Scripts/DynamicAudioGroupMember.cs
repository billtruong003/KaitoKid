...
namespace Fusion.Addons.DynamicAudioGroup
{
    ...
    public class DynamicAudioGroupMember : NetworkBehaviour
    {
        ...
        protected virtual bool ListenToMember(DynamicAudioGroupMember member)
        {
            if (member.Object == null)
            {
                return false;
            }
            if (listenedtoMembers.Contains(member)) return false;
            if (member.Listenable == false) return false;
            listenedtoMembers.Add(member);
            ListentoGroupId(member.GroupId);
            return true;
        }

        protected virtual bool StopListeningToMember(DynamicAudioGroupMember member)
        {
            if (member.Object == null)
            {
                return false;
