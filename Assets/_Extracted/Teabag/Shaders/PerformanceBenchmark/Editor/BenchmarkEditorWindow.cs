    ...
    public class BenchmarkEditorWindow : EditorWindow
    {
        ...
        private void DrawToolbar()
        {
            ...
            if (_benchmark != null)
            {
                ...
                // GPU Timing + Overlay toggles
                EditorGUILayout.BeginHorizontal();

                bool gpuBefore = _benchmark.enableGpuTiming;
                _benchmark.enableGpuTiming = EditorGUILayout.ToggleLeft(
                    "GPU Timing (⚠ ~100 FPS hit)", _benchmark.enableGpuTiming, GUILayout.Width(200));

                _benchmark.showOverlay = EditorGUILayout.ToggleLeft(
                    "Show Overlay", _benchmark.showOverlay, GUILayout.Width(120));

                EditorGUILayout.EndHorizontal();

                if (_benchmark.enableGpuTiming)
                {
                    EditorGUILayout.HelpBox(
                        "GPU Timing BẬT: FrameTimingManager force GPU sync mỗi frame → tụt ~50-100 FPS.\n" +
                        "Chỉ bật khi cần đo GPU cụ thể. Tắt để đo FPS chính xác.",
                        MessageType.Warning);
                }
            }
        }
