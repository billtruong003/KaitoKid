            ...
            if (interactionTip.CanMaintainInteraction())
            {
                ...
                for (int i = 0; i < interactionTipTargets.Count; i++)
                {
                    ...
                    if (interactionState.status != InteractionTipTargetStateStatus.NotInteracting)
                    {
                        ...
                        {
                            // Reject maintaining if finger is too far from the original UI target
                            float sqrDistance = (interactionTip.Origin - interactionTarget.transform.position).sqrMagnitude;
                            // 6f keeps the interaction active during normal hand movement
                            // while still preventing accidental reactivation when the finger drifts too far away.
                            float maxMaintainSqr = interactionTip.MaxStartInteractionDistance * interactionTip.MaxStartInteractionDistance * 6f;

                            if (sqrDistance > maxMaintainSqr)
                                continue; // Too far away; do not maintain this interaction
