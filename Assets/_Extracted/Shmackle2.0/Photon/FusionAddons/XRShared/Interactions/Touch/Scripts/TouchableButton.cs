...
namespace Fusion.XR.Shared.Core.Touch
{
    ...
    public class TouchableButton : MonoBehaviour, ITouchable, IHapticConsumer
    {
        ...
        public bool checkTouchDirection = false;
        ...
        public UnityEvent onButtonTouchStart;
        public UnityEvent onButtonTouchEnd;
        ...
        {
            if (checkTouchDirection)
            {
                // Check touch direction if it is the same direction as the rotation of this object
                Vector3 touchDirection = (transform.position - toucher.transform.position).normalized;
                float touchDot = Vector3.Dot(transform.forward, touchDirection);
                if (touchDot < 0)
                {
                    isButtonPressed = false;
                    return;
