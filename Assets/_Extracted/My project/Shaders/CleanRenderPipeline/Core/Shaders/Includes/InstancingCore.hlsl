...
// ============================================================================
// CleanRender — Instancing Core (WebGL-safe)
// Shared data structures + matrix construction for all instanced shaders
//
// NOTE: StructuredBuffer usage requires SHADER_API guards.
//       WebGL 2.0 (GLES3) does NOT support StructuredBuffer / SSBO.
//       Callers must wrap StructuredBuffer declarations like:
//       #if defined(UNITY_PROCEDURAL_INSTANCING_ENABLED) && !defined(SHADER_API_GLES3) && !defined(SHADER_API_GLES)
// ============================================================================
...
float4x4 QuatToInverseMatrix(float3 pos, float4 rot, float3 scl)
{
    ...
    float4x4 m;
    m[0] = float4(r0.x * is_val.x, r1.x * is_val.x, r2.x * is_val.x, 0);
    m[1] = float4(r0.y * is_val.y, r1.y * is_val.y, r2.y * is_val.y, 0);
    m[2] = float4(r0.z * is_val.z, r1.z * is_val.z, r2.z * is_val.z, 0);
...
//
// IMPORTANT: These macros are only valid on platforms that support
// StructuredBuffer (NOT WebGL/GLES3). The caller's Setup() function
