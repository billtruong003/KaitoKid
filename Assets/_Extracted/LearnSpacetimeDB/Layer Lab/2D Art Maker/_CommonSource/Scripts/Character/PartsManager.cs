...
namespace LayerLab.ArtMaker
{
    ...
    public class PartsManager : MonoBehaviour
    {
        ...
        {
            if (DemoControl.Instance != null && DemoControl.Instance.PanelParts != null)
            {
            ...
            {
                // Fallback: randomize all parts without DemoControl
                foreach (var kvp in _skinTypeToListMapping)
                {
                    if (kvp.Key == PartsType.None || kvp.Value.Count == 0) continue;
                    int randomIndex = UnityEngine.Random.Range(0, kvp.Value.Count);
                    ActiveIndices[kvp.Key] = randomIndex;
                    OnChangedParts?.Invoke(kvp.Key, randomIndex);
                }
            }
        ...
        {
            if (DemoControl.Instance == null || DemoControl.Instance.PanelParts == null)
                return true; // Default: allow hide when not in demo scene
