...
// ============================================================================
// CleanRender — Noise Library (v2) (WebGL-safe)
...
TriplanarUV ComputeTriplanarUV(float3 worldPos, float3 worldNormal, float scale, float sharpness)
{
    ...
    blendRaw = pow(blendRaw, float3(sharpness, sharpness, sharpness));
    ...
    float2 topScrollSpeed)
{
    ...
    result.sideNoise = (half)0.0;
    result.topNoise = (half)0.0;
    result.blend = float3(0, 0, 0);
