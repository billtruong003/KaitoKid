...
namespace Fusion.XR.Shared.Utils
{
    ...
    public class HideForLocalUser : NetworkBehaviour
    {
        ...
        void ConfigureCamera()
        {
            if (rig == null)
            {
                rig = HardwareRigsRegistry.GetHardwareRig();
            }
            int layer = LayerMask.NameToLayer(localUserLayer);
            if (hideLocalAvatar && layer != -1)
            {
                var camera = rig.Headset.gameObject.GetComponentInChildren<Camera>(true);
            ...
            if (localUserLayer != "")
            {
                ...
                else
                {
                    if (applyLayerToChildren)
                    {
                        ...
                        foreach (var renderer in gameObject.GetComponentsInChildren<Collider>())
                        {
                            renderer.gameObject.layer = layer;
                        }
                    }
